from bab.content.data_loader import load_card_database
from bab.systems.rewards import build_card_reward_pool


CARD_FILES = [
    "data/cards/witch_clerk_starter.json",
    "data/cards/witch_clerk_rewards.json",
]


def test_witch_clerk_reward_packages_load_and_exclude_generated_cards():
    card_database = load_card_database(CARD_FILES)
    reward_pool = build_card_reward_pool(card_database, card_class="witch_clerk")
    reward_ids = {card.id for card in reward_pool}

    generated_ids = {
        "hexed_stamp",
        "tea_charm",
        "ink_splatter",
        "office_familiar",
        "misfiled_hex",
    }

    assert generated_ids.issubset(card_database)
    assert generated_ids.isdisjoint(reward_ids)


def test_witch_clerk_reward_pool_contains_expected_packages():
    card_database = load_card_database(CARD_FILES)
    reward_pool = build_card_reward_pool(card_database, card_class="witch_clerk")
    tags_by_card = {card.id: set(card.tags) for card in reward_pool}

    assert any("hex" in tags and "paperwork" in tags for tags in tags_by_card.values())
    assert any("archive" in tags or "form" in tags for tags in tags_by_card.values())
    assert any("tea" in tags or "charm" in tags for tags in tags_by_card.values())
    assert any("ink" in tags or "stamp" in tags for tags in tags_by_card.values())
    assert any("familiar" in tags for tags in tags_by_card.values())


def test_witch_clerk_create_card_effects_reference_existing_cards():
    card_database = load_card_database(CARD_FILES)

    missing = []
    for card in card_database.values():
        for effect in card.effects:
            if effect.type == "create_card" and effect.card_id not in card_database:
                missing.append((card.id, effect.card_id))

    assert not missing


def test_witch_clerk_reward_cards_have_upgrades():
    card_database = load_card_database(CARD_FILES)
    reward_pool = build_card_reward_pool(card_database, card_class="witch_clerk")

    missing_upgrades = []
    for card in reward_pool:
        if card.rarity in {"common", "uncommon", "rare"}:
            if card.upgrades_to is None:
                missing_upgrades.append(card.id)
            elif card.upgrades_to not in card_database:
                missing_upgrades.append(card.id)

    assert not missing_upgrades


def test_witch_clerk_starter_cards_have_tags():
    card_database = load_card_database(CARD_FILES)

    for card_id in [
        "ritual_memo",
        "broom_shield",
        "suspicious_charm",
        "stamped_incantation",
    ]:
        assert card_database[card_id].tags
